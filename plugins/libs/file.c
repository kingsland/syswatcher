...
slurpfile (char * filename, char **buffer, int buflen)
{
   ...
   if (fd < 0)
      {
         plugin_print_log("slurpfile() open() error on file %s", filename);
   ...
   if (read_len < 0)
      {
         if (errno == EINTR)
            goto read;
         plugin_print_log("slurpfile() read() error on file %s", filename);
         close(fd);
         return SLURP_FAILURE;
      }
   else
   ...
   if (read_len == buflen)
      {
         if (dynamic) {
            ...
            *buffer = db;
            ...
            --read_len;
            plugin_print_log("slurpfile() read() buffer overflow on file %s", filename);
         }
      }
   db[read_len] = '\0';
...
update_file (timely_file *tf)
{
    ...
    if(timediff(&now,&tf->last_read) > tf->thresh) {
        ...
        if(rval == SLURP_FAILURE) {
            plugin_print_log("update_file() got an error from slurpfile() reading %s",
                    tf->name);
        } else {
            tf->last_read = now;
            if (tf->buffer == NULL) {
