...
slurpfile (char * filename, char **buffer, int buflen)
{
   ...
   if (fd < 0)
      {
         print_err("slurpfile() open() error on file %s", filename);
   ...
   if (read_len < 0)
      {
         if (errno == EINTR)
            goto read;
         print_err("slurpfile() read() error on file %s", filename);
         close(fd);
         return SLURP_FAILURE;
      }
   else
   ...
   if (read_len == buflen)
      {
         if (dynamic) {
            ...
            *buffer = db;
            ...
            --read_len;
            print_err("slurpfile() read() buffer overflow on file %s", filename);
         }
      }
   db[read_len] = '\0';
...
update_file (timely_file *tf)
{
    ...
    if(timediff(&now,&tf->last_read) > tf->thresh) {
        ...
        if(rval == SLURP_FAILURE) {
            print_err("update_file() got an error from slurpfile() reading %s",
                    tf->name);
        } else {
            tf->last_read = now;
            if (tf->buffer == NULL) {
