...
static net_dev_stats *hash_lookup(char *devname, size_t nlen)
{
    ...
    if ( stats == NULL )
    {
        print_err("unable to allocate memory for /proc/net/dev/stats in hash_lookup(%s,%zd)", name, nlen);
        free(name);
        return NULL;
    }
...
struct net_map *netdev_lookup(char *name)
{
    int idx;
    for (idx = 0; idx < sizeof(netdevs)/sizeof(struct net_map); idx++) {
        if (strcmp(netdevs[idx].name, name) == 0) {
            return &(netdevs[idx]);
        }
    }
    return NULL;
...
void update_ifdata ( char *caller )
{
            ...
            (proc_net_dev.last_read.tv_usec != stamp.tv_usec))
    {
        ...
        if ( t <  proc_net_dev.thresh) {
            print_err("update_ifdata(%s) - Dubious delta-t: %f", caller, t);
            return;
        }
        ...
        while (*p != 0x00)
        {
                    ...
                    strncmp (src, "bond", 4) && !vlan)
            {
                ...
                if ( rbi >= ns->rbi ) {
                    ...
                    print_info("update_ifdata(%s) - Overflow in rbi: %"PRI_STAT" -> %"PRI_STAT,caller,ns->rbi,rbi);
                ...
                if ( rpi >= ns->rpi ) {
                    ...
                    print_info("updata_ifdata(%s) - Overflow in rpi: %"PRI_STAT" -> %"PRI_STAT,caller,ns->rpi,rpi);
                ...
                if ( rbo >= ns->rbo ) {
                    ...
                    print_info("update_ifdata(%s) - Overflow in rbo: %"PRI_STAT" -> %"PRI_STAT,caller,ns->rbo,rbo);
                ...
                if ( rpo >= ns->rpo ) {
                    ...
                    print_info("update_ifdata(%s) - Overflow in rpo: %"PRI_STAT" -> %"PRI_STAT,caller,ns->rpo,rpo);
                ...
                netdev = netdev_lookup(ns->name);
                if (netdev == NULL) {
                    print_err("no such device %s\n", ns->name);
                } else {
                    netdev->speed = ns->bis + ns->bos;
                }
            }
            p = index (p, '\n') + 1;
        }
        ...
        if ((l_bin > 1.0e13) || (l_bout > 1.0e13) ||
            ...
            print_err("update_ifdata(%s): %g %g %g %g / %g", caller,
...
{
    if ( v > 100.0 ) {
        print_err( "file %s, line %d, fn %s: val > 100: %g ~ %g / %g = (%llu - %llu) / (%llu - %llu)\n", file, line, func, v, diff, dt, a, b, c, d );
        return 100.0;
    }
    else if ( v < 0.0 ) {
        print_err( "file %s, line %d, fn %s: val < 0: %g ~ %g / %g = (%llu - %llu) / (%llu - %llu)\n", file, line, func, v, diff, dt, a, b, c, d );
        return 0.0;
    }
    return v;
}
